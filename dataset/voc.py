...
class VOCDataset(data.Dataset):
    def __init__(self, 
        ...
        for line in open(os.path.join(rootpath, 'ImageSets', 'Main', f'{image_sets}.txt')):
...
if __name__ == "__main__":
    ...
    trans_config = {
        ...
        'use_ablu': True,
        # Mosaic & Mixup
        'mosaic_prob': default_config['mosaic'],
        'mixup_prob': default_config['mixup'],
        'mixup_scale': [0.5, 1.5]
    }
    transform, trans_cfg = build_transform(default_config, trans_config, 32, default_config['is_train'])

    dataset = VOCDataset(
        img_size=default_config['img_size'],
        data_dir=default_config['root'],
        image_sets=('trainval') if default_config['is_train'] else ('test'),
        trans_config=trans_config,
        transform=transform,
        is_train=default_config['is_train'],
    ...
    for i in range(len(dataset)):
        ...
        cv2.imshow('gt', image)
        # cv2.imwrite('./outjpg/'+str(i)+'.jpg', image)
        print('write image: ', i)
        cv2.waitKey(0)